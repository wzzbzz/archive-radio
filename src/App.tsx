...
function App() {
  ...
  useEffect(() => {
    if (selectedCollectionId) {
      // Load releases from Supabase instead of static manifest
      supabase
        .from('releases')
        .select('*')
        ...
        .then(({ data, error }) => {
          if (error) {
            console.error('Failed to load collection releases:', error);
            setCollectionReleases([]);
          } else {
            setCollectionReleases(data || []);
          }});
    }
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                {collections
                  .filter((collection: Collection) => {
                    ...
                    if (collection.is_virtual && collection.id === 'promoted') {
                ...
                return (
                  <>
                    <header className="mb-8">
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <p className="text-zinc-500 text-sm font-medium uppercase tracking-widest">
                          {collectionReleases.length} {collection.release_type}s • {Object.keys(tracks).filter(trackId => tracks[trackId].collection_id === selectedCollectionId).length} tracks
                        </p>
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex gap-4 mt-4 text-xs text-zinc-600">
                          <span className="uppercase tracking-wider">{collection.release_type}s</span>
                          <span>•</span>
